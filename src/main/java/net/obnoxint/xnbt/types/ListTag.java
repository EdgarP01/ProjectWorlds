...
public class ListTag extends AbstractListTag {
	...
	public boolean add(final NBTTag e) {
		...
		if (!isEmpty() && e.getHeader().getType() != getContentType()) {
			throw new IllegalArgumentException("NBTTag of type " + e.getHeader().getType() + " can't be added to ListTag because it already contains one or more elements of type " + getPayload().get(0).getHeader().getType());
		}
