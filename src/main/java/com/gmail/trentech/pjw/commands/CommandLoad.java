	...
	public CommandResult process(CommandSource source, String arguments) throws CommandException {
		...
		Task.builder().delayTicks(20).execute(c -> {
			...
			if (CommandCreate.worlds.contains(world.getWorldName())) {
				Optional<Location<World>> optionalLocation = TeleportManager.getRandomLocation(load.get(), 200);
				
				if(optionalLocation.isPresent()) {
					Location<World> location = optionalLocation.get();
					
					world.setSpawnPosition(new Vector3i().add(location.getBlockX(), location.getBlockY(), location.getBlockZ()));
